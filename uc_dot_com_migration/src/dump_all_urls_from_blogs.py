...
script_name = "dump_all_urls_from_blogs"
...
def main():
    ...
    rootdir = f"{config[ucutil.BLOGS_DIR]}/migrated/"
...
#            if ("WebSphereConfiguration" not in subdir): continue
    ...
    with open (f"{workfolder}/NEW_BLOGS-all_urls.csv", "w") as f:
        writer = csv.writer(f)
        writer.writerow(all_urls_header)
        writer.writerows(all_urls)
    with open (f"{workfolder}/NEW_BLOGS_UNIQUE-all_urls.csv", "w") as f:
        # writer = csv.writer(f)
