...
def get_list_of_doc_tabs(plugin, actdoc):
    ...
    # add Downloads tab!
    if (actdoc != ucutil.DOWNLOADS_DOCNAME): list_of_doc_tabs.append(ucutil.DOWNLOADS_DOCNAME)
...
def get_content_for_doc(doc, plugin, soup):
    ...
    doc_not_found = get_doc_not_found(plugin.get(doc))
...
def get_plugin_doc_folder_name(plugin) -> string:
    plugin_doc_folder_name = plugin.get(ucutil.NAME_PLUGIN_FOLDER_NAME).strip()
    if (not plugin_doc_folder_name): plugin.get(ucutil.NAME_PLUGIN_NAME)
        
    return plugin_doc_folder_name
...
def get_target_doc_path(config, plugin):
    target_doc_folder = get_plugin_doc_folder_name(plugin)
